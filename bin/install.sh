...
EOF
    echo -e "${BOLD}${CYAN}  Documentation generator for mq functions and macros${NC}"
    echo -e "${BLUE}  Generate documentation from mq source files${NC}"
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
}
...
get_latest_version() {
    local version
    version=$(curl -s "https://api.github.com/repos/$MQD_REPO/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
...
get_download_url() {
    ...
    echo "https://github.com/$MQD_REPO/releases/download/$version/mq-docs-${target}${ext}"
}

# Download checksums file
download_checksums() {
    local version="$1"
    local checksums_url="https://github.com/$MQD_REPO/releases/download/$version/checksums.txt"
...
# Download and install mq-docs
install_mqd() {
    local version="$1"
    local os="$2"
    local arch="$3"
    local download_url
    local binary_name="mq-docs"
    local ext=""
    local target=""

    if [[ "$os" == "windows" ]]; then
        ext=".exe"
        binary_name="mq-docs.exe"
        target="${arch}-pc-windows-msvc"
    elif [[ "$os" == "darwin" ]]; then
        target="${arch}-apple-darwin"
    else
    ...
    log "Downloading mq-docs $version for $os/$arch..."
    ...
    if ! curl -L --progress-bar "$download_url" -o "$temp_file"; then
        error "Failed to download mq-docs binary"
    fi

    # Verify checksum
    local release_binary_name="mq-docs-${target}${ext}"
    ...
    mv "$temp_file" "$MQD_BIN_DIR/$binary_name"
    chmod +x "$MQD_BIN_DIR/$binary_name"

    log "mq-docs installed successfully to $MQD_BIN_DIR/$binary_name"
}

# Add mq-docs to PATH by updating shell profile
update_shell_profile() {
    ...
    if echo "$PATH" | grep -q "$MQD_BIN_DIR"; then
        log "$MQD_BIN_DIR is already in PATH"
        return 0
    fi
    ...
    if [[ -n "$shell_profile" ]]; then
        ...
        if [[ "$shell_name" == "fish" ]]; then
            path_export="set -gx PATH \$PATH $MQD_BIN_DIR"
        else
            path_export="export PATH=\"\$PATH:$MQD_BIN_DIR\""
        fi

        if ! grep -q "$MQD_BIN_DIR" "$shell_profile" 2>/dev/null; then
            echo "" >> "$shell_profile"
            echo "# Added by mq-docs installer" >> "$shell_profile"
            echo "$path_export" >> "$shell_profile"
            log "Added $MQD_BIN_DIR to PATH in $shell_profile"
        else
            warn "$MQD_BIN_DIR already exists in $shell_profile"
        fi
    else
        warn "Could not detect shell profile to update"
        warn "Please manually add $MQD_BIN_DIR to your PATH"
    fi
}

# Verify installation
verify_installation() {
    # Check mq-docs installation
    if [[ -x "$MQD_BIN_DIR/mq-docs" ]] || [[ -x "$MQD_BIN_DIR/mq-docs.exe" ]]; then
        log "✓ mq-docs installation verified"
        log "Installation verification successful!"
        return 0
    else
        error "mq-docs installation verification failed"
    fi
}
...
show_post_install() {
    echo ""
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BOLD}${GREEN}✨ mq-docs installed successfully! ✨${NC}"
    echo -e "${PURPLE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    ...
    echo -e "     ${CYAN}mq-docs --version${NC}"
    echo ""
    echo -e "  ${YELLOW}3.${NC} Get help:"
    echo -e "     ${CYAN}mq-docs --help${NC}"
    echo ""
    echo -e "${BOLD}${CYAN}⚡ Quick Examples:${NC}"
    echo -e "  ${GREEN}▶${NC} ${CYAN}mq-docs generate src/       # Generate docs from mq source files${NC}"
    echo -e "  ${GREEN}▶${NC} ${CYAN}mq-docs --help              # Show available commands${NC}"
    echo ""
    echo -e "${BOLD}${CYAN}📚 Learn More:${NC}"
    echo -e "  ${GREEN}▶${NC} Repository: ${BLUE}https://github.com/$MQD_REPO${NC}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --help|-h)
            echo "mq-docs installation script"
            echo ""
